package com.vooda.system.controller.head;
...
import com.vooda.frame.mail.SimpleMailSender;
import com.vooda.frame.util.AppUtil;
import com.vooda.frame.util.Const;
import com.vooda.frame.util.SmsUtil;
import com.vooda.frame.util.Tools;
import com.vooda.system.service.appuser.AppuserService;
...
public class HeadController extends BaseController {
	...
	public Object sendEmail() {
		...
		if (null != strEMAIL && !"".equals(strEMAIL)) {
			String strEM[] = strEMAIL.split(",vooda,");
			if (strEM.length == 4) {
				if ("yes".endsWith(isAll)) {
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception {
		...
		if (null != strSMS1 && !"".equals(strSMS1)) {
			String strS1[] = strSMS1.split(",vooda,");
			if (strS1.length == 2) {
		...
		if (null != strSMS2 && !"".equals(strSMS2)) {
			String strS2[] = strSMS2.split(",vooda,");
			if (strS2.length == 2) {
	...
	public ModelAndView saveU(PrintWriter out) throws Exception {
		...
		Tools.writeFile(Const.EMAIL, pd.getString("SMTP") + ",vooda," + pd.getString("PORT") + ",vooda," + pd.getString("EMAIL") + ",vooda," + pd.getString("PAW")); //写入邮件服务器配置
		Tools.writeFile(Const.SMS1, pd.getString("SMSU1") + ",vooda," + pd.getString("SMSPAW1")); //写入短信1配置
		Tools.writeFile(Const.SMS2, pd.getString("SMSU2") + ",vooda," + pd.getString("SMSPAW2")); //写入短信2配置
		mv.addObject("msg", "OK");
