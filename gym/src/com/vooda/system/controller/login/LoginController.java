package com.vooda.system.controller.login;
...
import com.vooda.frame.util.RightsHelper;
import com.vooda.frame.util.Tools;
import com.vooda.system.entity.Menu;
import com.vooda.system.entity.Role;
...
//						.toString(); // 密码加密
	...
	public ModelAndView defaultPage() throws Exception{
		...
		mv.addObject("day", mUserService.userRegistByDay());
		mv.addObject("week", mUserService.userRegistByWeek());
		mv.addObject("month", mUserService.userRegistByMonth());
		mv.addObject("total", mUserService.userRegistTotal());
	...
	@ResponseBody
    public String indexCountList() throws Exception{
        PageData pd = this.getPageData();
        List<PageData> varList = mUserService.userRegistInfo(pd);
        /*int xtimes = 12;
        if(pd.containsKey("month") && !"0".equals(pd.getStringOf("month"))){
        	Calendar cal = Calendar.getInstance();
        	cal.setTime(new SimpleDateFormat("yyyyMdd").parse(
        			pd.getStringOf("year") + pd.getStringOf("month") + "01"));
        	xtimes = cal.getActualMaximum(Calendar.DATE);
        }
    	StringBuilder times = new StringBuilder();
    	times.append("[");
    	for(int i = 1; i <= xtimes; i++){
    		times.append(i).append(i == xtimes ? "]" : ",");
    	}
    	json.accumulate("scantimes", times.toString());*/
//        json.accumulate("varList", varList);
        JSONObject json = new JSONObject();
        if(varList != null && !varList.isEmpty()){
            StringBuilder times = new StringBuilder();
            StringBuilder scansum = new StringBuilder();
            times.append("[");
            scansum.append("[");
            for(int i=0; i<varList.size(); i++){
                times.append(varList.get(i).getString("regist_time")).append(i == varList.size() -1 ? "]" : ",");
                scansum.append(varList.get(i).getStringOf("regsum")).append(i == varList.size() -1 ? "]" : ",");
            }
            json.accumulate("scantimes", times.toString());
            json.accumulate("scansum", scansum.toString());
        }
