...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="pictures/list.action" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>/pictures/goAdd.action';
			 diag.Width = 800;
			 diag.Height = 490;
		...
		function del(Id,PATH){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>/pictures/delete.action?PICTURES_ID="+Id+"&PATH="+PATH+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>/pictures/goEdit.action?PICTURES_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 465;
			 diag.CancelEvent = function(){ //关闭事件
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>/pictures/deleteAll.action?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>/pictures/excel.action';
		}
		</script>
		<style type="text/css">
