...
<script type="text/javascript">
	...
	function addmenu(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增菜单";
		 diag.URL = '<%=basePath%>/menu/toAdd.action';
		 diag.Width = 223;
		 diag.Height = 256;
	...
	function editmenu(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑菜单";
		 diag.URL = '<%=basePath%>/menu/toEdit.action?MENU_ID='+menuId;
		 diag.Width = 223;
		 diag.Height = 256;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function editTb(menuId){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑图标";
		 diag.URL = '<%=basePath%>/menu/toEditicon.action?MENU_ID='+menuId;
		 diag.Width = 530;
		 diag.Height = 150;
		 diag.CancelEvent = function(){ //关闭事件
			if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function delmenu(menuId,isParent){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>/menu/del.action?MENU_ID="+menuId+"&guid="+new Date().getTime();
	...
	function openClose(menuId,curObj,trIndex){
		...
		if(txt=="展开"){
			...
			var url = "<%=basePath%>menu/sub.action?MENU_ID="+menuId+"&guid="+new Date().getTime();
			$.get(url,function(data){
				if(data.length>0){
					var html = "";
