				...
				<c:choose>
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="user" varStatus="vs">
							...
							<tr>
								<td>${user.braceletid }</td>
								<td>	
									<c:choose>
										<c:when test="${user.remindtype eq '0'}">提醒一次</c:when>
										<c:when test="${user.remindtype eq '1'}">天</c:when>
										<c:when test="${user.remindtype eq '2'}">周</c:when>
										<c:when test="${user.remindtype eq '3'}">月</c:when>
										<c:when test="${user.remindtype eq '4'}">年</c:when>
									</c:choose>
								</td>
								<td>${user.remindtime }</td>
								<td>
									<c:choose>
										<c:when test="${user.statu eq '1'}">提醒</c:when>
										<c:when test="${user.statu eq '2'}">不提醒</c:when>
									</c:choose>
								</td>
							</tr>
