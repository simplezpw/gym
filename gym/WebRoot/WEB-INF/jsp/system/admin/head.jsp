...
<div class="navbar navbar-inverse">
		  <div class="navbar-inner">
		   <div class="container-fluid">
			  <a class="brand" href="login_index.action"><small><i class="icon-leaf"></i> ${pd.SYSNAME}</small> </a>
			  
			  <ul class="nav ace-nav pull-right">
					<li class="light-blue user-profile">
						<a class="user-menu dropdown-toggle" href="javascript:alert('预留功能,待开发');" data-toggle="dropdown">
							<img alt="vooda" src="static/avatars/user.jpg" class="nav-user-photo" />
							<span id="user_info">
		...
		<script type="text/javascript">
			...
			$(function(){
				...
				$("#skin-colorpicker").ace_colorpicker().on("change",function(){
					...
					hf(b);
					var url = '<%=basePathh%>/head/setSKIN.action?SKIN='+b+'&tm='+new Date().getTime();
			...
			function getUname(){
				$.ajax({
					type: "POST",
					url: '<%=basePathh%>/head/getUname.action?tm='+new Date().getTime(),
			    	data: encodeURI(""),
					dataType:'json',
			...
			function editUserH(){
				 ...
				 diag.Drag=true;
				 diag.Title ="个人资料";
				 diag.URL = '<%=basePathh%>/user/goEditU.action?USER_ID='+USER_ID+'&fx=head';
				 diag.Width = 225;
				 diag.Height = 389;
			...
			function editSys(){
				 ...
				 diag.Drag=true;
				 diag.Title ="系统设置";
				 diag.URL = '<%=basePathh%>/head/goSystem.action';
				 diag.Width = 600;
				 diag.Height = 596;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			function productCode(){
				 ...
				 diag.Drag=true;
				 diag.Title ="代码生成器";
				 diag.URL = '<%=basePathh%>/head/goProductCode.action';
				 diag.Width = 800;
				 diag.Height = 450;
				 diag.CancelEvent = function(){ //关闭事件
					changeui();
			...
			function zidian(){
				 ...
				 diag.Drag=true;
				 diag.Title ="数据字典";
				 diag.URL = '<%=basePathh%>/dictionaries.action?PARENT_ID=0';
				 diag.Width = 799;
				 diag.Height = 460;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			function menu(){
				 ...
				 diag.Drag=true;
				 diag.Title ="菜单编辑";
				 diag.URL = '<%=basePathh%>/menu.action';
				 diag.Width = 720;
				 diag.Height = 390;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			//切换菜单
			function changeMenu(){
				window.location.href='<%=basePathh%>login_index.action?changeMenu=yes';
			}
