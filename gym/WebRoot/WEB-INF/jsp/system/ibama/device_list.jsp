...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="muser/devicelist.action" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="userid" value="${pd.userid }" placeholder="设备绑定手机号" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="deviceid" value="${pd.deviceid }" placeholder="设备号" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="name" value="${pd.name }" placeholder="关系(爸爸/妈妈等)" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th>序号</th>
						<th>设备号</th>
						<th style="word-wrap:break-word;">绑定用户id(第一个首绑)</th>
						<th>设备名称</th>
						<th>设备版本号</th>
						<th>设备使用者与用户关系</th>
					...
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<td>${user.deviceid }</td>
								<td style="word-wrap:break-word;word-break:break-all;">${user.userids }</td>
								<td>${user.name }</td>
								<td>${user.app_version }</td>
								<td><a href="<%=basePath%>/muser/healthDetail.action?deviceid=${user.deviceid}" title="健康详情">${user.names}</a></td>
								<td>
									<a title="设备详情" class="btn btn-small btn-info" onclick="userDetail(${user.id});"><i class="icon-info-sign"></i></a>
								</td>
							</tr>
