...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="muser/list.action" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="priid" type="text" name="id" style="width:120px;" value="${pd.id }" placeholder="用户id" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="userid" value="${pd.userid }" placeholder="手机号" />
							<i id="nav-search-icon" class="icon-search"></i>
						</span>
					</td>
					<td><input class="span10 date-picker" name="stime" id="stime"  value="${pd.stime}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期" title="注册时间开始"/></td>
					<td><input class="span10 date-picker" name="etime" id="etime"  value="${pd.etime}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="注册时间结束"/></td>
					...
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="user" varStatus="vs">
							...
							<tr>
								...
								<td>${user.deviceids }</td>
								<td>${user.app_version}</td>
								<td>
									<a title="用户详情" class="btn btn-small btn-info" onclick="userDetail(${user.userid});"><i class="icon-info-sign"></i></a>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function userDetail(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = 'muser/userDetail.action?user_id='+user_id;
			 diag.Width = 600;
			 diag.Height = 400;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
