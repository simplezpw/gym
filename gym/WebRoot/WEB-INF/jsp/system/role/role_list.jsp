  ...
  <div class="row-fluid">
				...
				<td style="width:69px;"><a href="javascript:addRole();" class="btn btn-small btn-success">新增</a></td>
					...
					<c:forEach items="${roleList}" var="role" varStatus="vs">
						<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
							<a href="role.action?ROLE_ID=${role.ROLE_ID }" style="text-decoration:none; display:block;"><li class=" icon-group"></li>&nbsp;<font color="#666666">${role.ROLE_NAME }</font></a>
						</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function addRole(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增组";
			 diag.URL = 'role/toAdd.action?parent_id=0';
			 diag.Width = 222;
			 diag.Height = 90;
		...
		function addRole2(pid){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增角色";
			 diag.URL = 'role/toAdd.action?parent_id='+pid;
			 diag.Width = 222;
			 diag.Height = 90;
			 diag.CancelEvent = function(){ //关闭事件
				if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function editRole(ROLE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = 'role/toEdit.action?ROLE_ID='+ROLE_ID;
			 diag.Width = 222;
			 diag.Height = 90;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delRole(ROLE_ID,msg,ROLE_NAME){
			bootbox.confirm("确定要删除["+ROLE_NAME+"]吗?", function(result) {
				if(result) {
					var url = "role/delete.action?ROLE_ID="+ROLE_ID+"&guid="+new Date().getTime();
					top.jzts();
		...
		function kf_qx1(id,kefu_id,msg){
			...
			if(qxhc1 == 'checked'){
				...
				var url = "role/kfqx.action?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function c1(id,msg,value,kefu_id){
				...
				var url = "role/gysqxc.action?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function editRights(ROLE_ID){
			 ...
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = 'role/auth.action?ROLE_ID='+ROLE_ID;
			 diag.Width = 280;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function roleButton(ROLE_ID,msg){
			...
			if(msg == 'add_qx'){
			 ...
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = 'role/button.action?ROLE_ID='+ROLE_ID+'&msg='+msg;
			 diag.Width = 200;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
