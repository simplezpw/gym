...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="happuser/listUsers.action" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function sendEmail(EMAIL){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送电子邮件";
			 diag.URL = '<%=basePath%>/head/goSendEmail.action?EMAIL='+EMAIL+'&msg=appuser';
			 diag.Width = 600;
			 diag.Height = 470;
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增会员";
			 diag.URL = 'happuser/goAddU.action';
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function editUser(user_id){
			 ...
			 diag.Drag=true;
			 diag.Title ="会员资料";
			 diag.URL = 'happuser/goEditU.action?USER_ID='+user_id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "happuser/deleteU.action?USER_ID="+userId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: 'happuser/deleteAllU.action?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>/happuser/excel.action?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID+'&STATUS='+STATUS;
		}
		</script>
