...
<script type="text/javascript">
	...
	function add(PARENT_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>/dictionaries/toAdd.action?PARENT_ID='+PARENT_ID;
		 diag.Width = 223;
		 diag.Height = 175;
	...
	function edit(ZD_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>/dictionaries/toEdit.action?ZD_ID='+ZD_ID;
		 diag.Width = 223;
		 diag.Height = 175;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	function del(ZD_ID){
		...
		if(flag){
			top.jzts();
			var url = "<%=basePath%>/dictionaries/del.action?ZD_ID="+ZD_ID+"&guid="+new Date().getTime();
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/dictionaries.action" method="post" name="userForm" id="userForm">
	...
	<table>
		<tr>
			...
			<c:if test="${pd.PARENT_ID != '0'}">
				...
				<td style="vertical-align:top;"><a href="<%=basePath%>/dictionaries.action?PARENT_ID=0" class="btn btn-mini btn-purple" title="查看">顶级<i class="icon-arrow-right  icon-on-right"></i></a></td>
				<c:forEach items="${varsList}" var="var" varStatus="vsd">
				<td style="vertical-align:top;"><a href="<%=basePath%>/dictionaries.action?PARENT_ID=${var.ZD_ID }" class="btn btn-mini btn-purple" title="查看">${var.NAME }<i class="icon-arrow-right  icon-on-right"></i></a></td>
				</c:forEach>
				</c:when>
				<c:otherwise>
				</c:otherwise>
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty varList}">
				...
				<td class='center'><a href="<%=basePath%>/dictionaries.action?PARENT_ID=${var.ZD_ID }" title="查看下级"><i class="icon-arrow-right  icon-on-right"></i>&nbsp;${var.NAME }</a></td>
		...
		<table style="width:100%;">
			<tr>
				...
				<c:if test="${pd.PARENT_ID != '0'}">
					<td style="vertical-align:top;" class="left"><a class="btn btn-small btn-info" onclick="location.href='<%=basePath%>/dictionaries.action?PARENT_ID=${pdp.PARENT_ID }';">返回</a></td>
				</c:if>
